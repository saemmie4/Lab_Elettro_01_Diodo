...
void fit_diodato()
{
    ...
    // TF1 * fit_Si = new TF1("fit_Si", "[0]*(exp(x/[1]) - 1)", 0, 500);
    TF1 * fit_Si = new TF1("fit_Si", "[0]*(TMath::Log(x/[1] + 1))", 0.05, 1);
    TF1 * fit_Ge = new TF1("fit_Ge", "[0]*(exp(x/[1]) - 1)", 0, 400);
    
    fit_Si->SetParameter(1, 1e-6);
    fit_Si->SetParameter(0,550);
    // fit_Ge->SetParameter(0, 1);
    // fit_Ge->SetParameter(1,200);
